...
var message = ["", "ok", "good", "very good", "great", "excellent"];
...
function go() {
    ...
    document.body.addEventListener('touchmove', function(evetn) {
...
function setColor(x, y, c) {
    ...
    if (x < 8) {
        var c0 = tiles[x + 2][y].color;
        var c0 = tiles[x + 1][y].color;
        flag &= !(c0 == c1 && c1 == c);
    }
    if (1 < y) {
        var c0 = tiles[x][y - 2].color;
        var c0 = tiles[x][y - 1].color;
        flag &= !(c0 == c1 && c1 == c);
    }
    if (y < 8) {
        var c0 = tiles[x][y + 2].color;
...
function mymouseup(e) {
    ...
    if (nx > 11 || ny > 11 || nx < 0 || ny < 0 || tiles[sx][sy].moving || tiles[nx][ny].moving) {
        return;
    }
